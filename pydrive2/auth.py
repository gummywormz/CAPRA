"""NOTICE:
This file has been modified from the original PyDrive in order to serve CAPRA's
purposes better. This will probably become an enhanced version but for right 
now this is it. This file is the only file that has been changed:

The oauth_scope variable has been hard coded because it seems 
that settings.yaml is not recognized / taking effect.

Authorize returns the service instance so that you may use it with other APIs
and Google Drive functions instead of just the ones provided by default
"""
...
class GoogleAuth(ApiAttributeMixin, object):
  ...
  DEFAULT_SETTINGS = {
      ...
      'oauth_scope': ['https://www.googleapis.com/auth/drive.readonly https://www.googleapis.com/auth/gmail.compose']
      }
